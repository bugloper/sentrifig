...
module SeliseSentry
  class TasksTest < ActiveSupport::TestCase
    setup do
      ...
      load File.expand_path("../../lib/tasks/selise_sentry.rake", __dir__)
    end
    ...
    test "status shows the default state and SDK readiness" do
      output = run_task("selise_sentry:status")
    ...
    test "disable and enable share the state with the API and UI" do
      output = run_task("selise_sentry:disable")
    ...
    test "install copies the migration into the application and prints next steps" do
      Dir.mktmpdir do |dir|
        ActiveRecord::Tasks::DatabaseTasks.stub(:migrations_paths, [dir]) do
          output = run_task("selise_sentry:install")
          assert_includes output, "copied"
          assert_includes output, "bin/rails db:migrate"
          assert_includes output, 'mount SeliseSentry::Engine => "/selise-sentry"'
          assert_includes output, "SELISE_SENTRY_USERNAME"

          copied = Dir[File.join(dir, "*_create_selise_sentry_settings.selise_sentry.rb")]
          assert_equal 1, copied.size
          assert_includes File.read(copied.first), "create_table :selise_sentry_settings"
