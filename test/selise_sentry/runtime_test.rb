...
module SeliseSentry
  class RuntimeTest < ActiveSupport::TestCase
    ...
    test "update! persists and applies immediately" do
      ...
      assert_includes @log.string, "[selise-sentry] Sentry disabled environment=production by=alice"
    end
    ...
    test "a store failure before anything was loaded falls back to the default" do
      @store.failure = ActiveRecord::StatementInvalid.new("no such table: selise_sentry_settings")
      assert @runtime.enabled?
